/**
 * Stage 122 packaged-app smoke — Safe Scaffold File Tree Preview after Stage 121.
 ...
 * then refreshes safety and generates file-tree preview via IPC.
 */
...
## Project Brief
Stage 122 disposable smoke blueprint.
## Product Requirements
Verify packaged Safe Scaffold File Tree Preview.
## User Stories
As a user I want paths-only file tree preview.
## Feature Roadmap
Phase 1: packaging validation only.
## Data Model
N/A.
## Screen / Workflow Flow
Build tab file-tree preview.
## Architecture Plan
...
## 1. Analysis
Stage 122 Blueprint regression.

## 2. Plan
Confirm Blueprint still works after Stage 121 packaging.
...
No Apply Patch. File-tree preview paths only.
...
function injectSafeTarget(emptyDir, check) {
  ...
  // Clear preview so generate is fresh
...
try {
  ...
  report.bundle = {
    stage121Wiring:
      ...
      bundleText.includes("Generate File Tree Preview") &&
      bundleText.includes("Copy File Tree Preview") &&
      bundleText.includes("Clear File Tree Preview") &&
  ...
  if (!report.bundle.stage121Wiring) fail("Bundle missing Stage 121 wiring");
  if (!report.bundle.noApplyPatch) fail("Bundle unexpectedly contains Apply Patch");
  if (!report.bundle.guideNote) fail("Bundle missing Guide file-tree note");
  ...
  const evaluatePreconditions = builderMod.evaluateFileTreePreviewPreconditions;
  report.pathSafety = {
  ...
  report.preconditions = {
    noBlueprint: !evaluatePreconditions({
    ...
    noTarget: !evaluatePreconditions({
      blueprintImported: true,
      taskCardCount: 1,
    ...
    blockedTarget: !evaluatePreconditions({
      blueprintImported: true,
      taskCardCount: 1,
      targetFolderPath: "D:\\t",
    ...
    staleTarget: !evaluatePreconditions({
      blueprintImported: true,
      taskCardCount: 1,
      targetFolderPath: "D:\\t",
      targetSafetyStatus: "safe",
    ...
    cautionOk: evaluatePreconditions({
      blueprintImported: true,
      taskCardCount: 1,
      targetFolderPath: "D:\\t",
  ...
  if (Object.values(report.preconditions).some((v) => !v)) {
    fail("Precondition matrix failed");
  }

  const cautionBuilt = buildPreview({
    blueprintImported: true,
  ...
  report.cautionPreview = {
    ...
    hasSections: [
      ...
      "## Safety Boundaries",
      "## Next Step",
    ].every((s) => (cautionBuilt.record?.markdown ?? "").includes(s)),
    relativeOnly: (cautionBuilt.record?.proposedRelativePaths ?? []).every(
      (p) => validate(p) === null,
    ),
    noContentsBodies:
      !/function\s+|console\.log|import\s+React/i.test(
        cautionBuilt.record?.markdown ?? "",
      ),
  };
  if (
    !report.cautionPreview.generated ||
    !report.cautionPreview.hasWarning ||
    !report.cautionPreview.hasSections ||
    !report.cautionPreview.relativeOnly
  ) {
    fail("Caution preview structural checks failed");
  }

  const emptyDir = fs.mkdtempSync(path.join(os.tmpdir(), "nttc-s122-safe-"));
  ...
  report.buildUi = {
    ...
    generateBtn: buildButtons.some((b) => /^Generate File Tree Preview$/i.test(b)),
    copyBtn: buildButtons.some((b) => /^Copy File Tree Preview$/i.test(b)),
    clearBtn: buildButtons.some((b) => /^Clear File Tree Preview$/i.test(b)),
    noGenerateContents: !buildButtons.some((b) => /^Generate Contents$/i.test(b)),
    noCreateFiles: !buildButtons.some((b) => /^Create Files$/i.test(b)),
    noScaffold: !buildButtons.some((b) => /^Scaffold$/i.test(b)),
    noWrite: !buildButtons.some((b) => /^Write$/i.test(b) || /^Write Files$/i.test(b)),
  ...
  await evaluate(
    `window.nttc.setBlueprintIntake(${JSON.stringify({
      projectIdea: "Stage 122 file tree packaging smoke.",
      projectType: "web-app",
  ...
  let s = await snap();
  const preview = s.safeScaffoldFileTreePreview?.saved;
  report.preview = {
    exists: Boolean(preview),
    stale: Boolean(preview?.stale),
    pathCount: preview?.proposedRelativePaths?.length ?? 0,
    relativeOnly: (preview?.proposedRelativePaths ?? []).every(
      (p) => typeof p === "string" && !/[A-Za-z]:/.test(p) && !p.includes("..") && !p.startsWith("/"),
    ),
    hasHeader: /# NTTC Safe Scaffold File Tree Preview/.test(preview?.markdown ?? ""),
    hasStatus: /## Status/.test(preview?.markdown ?? ""),
    hasTree: /## Proposed File Tree/.test(preview?.markdown ?? ""),
    hasSafety: /## Safety Boundaries/.test(preview?.markdown ?? ""),
  ...
  await evaluate(`window.nttc.recordCopySafeScaffoldFileTreePreview()`);
  await new Promise((r) => setTimeout(r, 300));

  // Stale after regenerate task cards
  await evaluate(`window.nttc.generateBlueprintPhaseTaskCards()`);
  await new Promise((r) => setTimeout(r, 600));
  s = await snap();
  report.staleAfterTaskCards = Boolean(
    s.safeScaffoldFileTreePreview?.saved?.stale,
  );

  // Regenerate fresh preview for persistence test
  await evaluate(`window.nttc.refreshSafeScaffoldTargetSafety()`);
  await evaluate(`window.nttc.generateSafeScaffoldFileTreePreview()`);
  await new Promise((r) => setTimeout(r, 600));
  s = await snap();
  const markdownForPersist = s.safeScaffoldFileTreePreview?.saved?.markdown ?? "";
  report.previewFresh = Boolean(
    s.safeScaffoldFileTreePreview?.saved &&
      !s.safeScaffoldFileTreePreview.saved.stale,
  );
  ...
  // Ensure a current preview exists before restart persistence
  await evaluate(`window.nttc.refreshSafeScaffoldTargetSafety()`);
  await evaluate(`window.nttc.generateSafeScaffoldFileTreePreview()`);
  await new Promise((r) => setTimeout(r, 500));
  s = await snap();
  const beforeRestartMarkdown =
    s.safeScaffoldFileTreePreview?.saved?.markdown?.slice(0, 80) ?? "";
  ...
  report.persistenceRestore = {
    hasPreview: Boolean(s.safeScaffoldFileTreePreview?.saved?.markdown),
    stale: Boolean(s.safeScaffoldFileTreePreview?.saved?.stale),
    noFileContentsField: !JSON.stringify(
      s.safeScaffoldFileTreePreview?.saved ?? {},
    ).includes("fileContents"),
    markdownSnippetMatches: Boolean(
      beforeRestartMarkdown &&
        (s.safeScaffoldFileTreePreview?.saved?.markdown ?? "").includes(
          beforeRestartMarkdown.slice(0, 40),
        ),
    ),
  };

  await conn.evaluate(`window.nttc.clearSafeScaffoldFileTreePreview()`);
  await new Promise((r) => setTimeout(r, 400));
  s = await conn.snap();
  report.persistenceClear = {
    cleared: !s.safeScaffoldFileTreePreview?.saved,
  };
  conn.ws.close();
  spawn("taskkill", ["/F", "/IM", "New Type Tech Coder.exe"], { stdio: "ignore" });
  await new Promise((r) => setTimeout(r, 2000));
  ...
  report.persistenceClearedPersists = {
    stillCleared: !s.safeScaffoldFileTreePreview?.saved,
  };
  conn.ws.close();
  spawn("taskkill", ["/F", "/IM", "New Type Tech Coder.exe"], { stdio: "ignore" });
  ...
  if (!report.buildUi.fileTreeSection) fail("File Tree Preview section missing");
  if (!report.buildUi.generateBtn) fail("Generate File Tree Preview missing");
  if (!report.buildUi.copyBtn) fail("Copy File Tree Preview missing");
  if (!report.buildUi.clearBtn) fail("Clear File Tree Preview missing");
  if (!report.buildUi.noGenerateContents) fail("Generate Contents button found");
  ...
  if (!report.preview.exists) fail("File tree preview did not generate");
  ...
  if (!report.staleAfterTaskCards) fail("Preview did not stale after task cards");
  ...
  if (!report.guide.fileTreeNote) fail("Guide missing file-tree note");
  if (!report.reports.workflowFileTree) fail("Workflow missing File Tree Preview step");
  ...
  if (!report.persistenceRestore.hasPreview) fail("Preview did not restore");
  if (!report.persistenceRestore.stale) fail("Restored preview should be stale");
  if (!report.persistenceClear.cleared) fail("Clear preview failed");
  if (!report.persistenceClearedPersists.stillCleared) {
    fail("Cleared preview reappeared after restart");
  }
