/**
 * Stage 93 packaged-app smoke (CDP) — Blueprint Task Reconciliation focus.
...
try {
  ...
  report.ui = {
    ...
    generateReconBtn: /Generate Reconciliation Report/i.test(bodyText),
    copyReconBtn: /Copy Reconciliation Report/i.test(bodyText),
    clearReconBtn: /Clear Reconciliation Report/i.test(bodyText),
  };
  ...
  let s = await snap();
  const cardsMd = s.blueprint?.phaseTaskCards?.saved?.markdown ?? "";
  const p1a = s.blueprint?.phaseTaskCards?.saved?.cards?.find((c) => c.id === "P1A");
  report.contractFields = {
    cardProduces: Boolean(p1a?.producesCreates?.trim()),
    cardConsumes: Boolean(p1a?.consumesDependsOn?.trim()),
    cardInterfaces: Boolean(p1a?.interfacesContracts?.trim()),
    anyCardHasContracts: (s.blueprint?.phaseTaskCards?.saved?.cards ?? []).some(
      (c) =>
        Boolean(c.producesCreates?.trim()) &&
        Boolean(c.consumesDependsOn?.trim()) &&
        Boolean(c.interfacesContracts?.trim()),
    ),
  };

  await evaluate(`window.nttc.generateBlueprintTaskReconciliation()`);
  await new Promise((r) => setTimeout(r, 400));
  const advisorBeforeRecon = (await snap()).advisorResponse;
  s = await snap();
  const recon = s.blueprint?.taskReconciliation?.saved;
  const reconMd = recon?.markdown ?? "";
  report.reconciliationGenerate = {
    saved: Boolean(recon),
    titleOk: reconMd.includes("# NTTC Blueprint Task Reconciliation Report"),
    sectionsOk: RECON_SECTIONS.every((sec) => reconMd.includes(sec)),
    recommendation: recon?.recommendation ?? null,
    recommendationKnown: RECOMMENDATIONS.includes(recon?.recommendation),
    generateLog: (s.actionLog ?? []).some((e) =>
      /Reconciliation report generated/i.test(e.message),
    ),
    noAdvisorCall: advisorBeforeRecon === s.advisorResponse,
    noCodeAiCall: !s.codeContextAi?.saved,
  };

  await evaluate(`window.nttc.recordCopyBlueprintTaskReconciliation()`);
  s = await snap();
  report.reconciliationCopy = {
    copyLog: (s.actionLog ?? []).some((e) =>
      /Reconciliation report copied/i.test(e.message),
    ),
  };

  await evaluate(`window.nttc.clearBlueprintTaskReconciliation()`);
  s = await snap();
  report.reconciliationClear = {
    cleared: !s.blueprint?.taskReconciliation?.saved,
    clearLog: (s.actionLog ?? []).some((e) =>
      /Reconciliation report cleared/i.test(e.message),
    ),
  };
  ...
  await new Promise((r) => setTimeout(r, 1500));
  s = await snap();
  report.reconciliationHistoryRestore = {
    restored: Boolean(s.blueprint?.taskReconciliation?.saved),
    recommendation: s.blueprint?.taskReconciliation?.saved?.recommendation ?? null,
  };
  ...
  await new Promise((r) => setTimeout(r, 400));
  s = await snap();
  report.reconciliationStale = {
    regenerated: s.blueprint?.phaseTaskCards?.saved?.generatedAt !== genAtBefore,
    stale: Boolean(s.blueprint?.taskReconciliation?.saved?.stale),
    staleLog: (s.actionLog ?? []).some((e) =>
      /Reconciliation report stale/i.test(e.message),
    ),
  };

  await evaluate(`window.nttc.generateBlueprintTaskReconciliation()`);
  await evaluate(`window.nttc.setBlueprintPhaseTaskCardStatus("P1A", "reviewed")`);
  await evaluate(`window.nttc.clearTaskImplementationReport()`);
  s = await snap();
  const statusReconMd = s.blueprint?.taskReconciliation?.saved?.markdown ?? "";
  report.statusConsistency = {
    hasStatusSection: statusReconMd.includes("## Status Consistency"),
    reviewedNoReportWarning:
      /reviewed/i.test(statusReconMd) &&
      /implementation report|status inconsistency|no implementation/i.test(statusReconMd),
  };
  ...
  await evaluate(`window.nttc.generateBlueprintTaskReconciliation()`);
  s = await snap();
  const driftMd = s.blueprint?.taskReconciliation?.saved?.markdown ?? "";
  report.contractDrift = {
    hasDriftSection: driftMd.includes("## Contract Drift"),
    handoffTaskMismatch: /handoff|mismatch|drift/i.test(driftMd),
  };
  ...
  const reconFile = preview?.files?.find((f) => f.fileName === "TASK_RECONCILIATION.md");
  report.planningPreview = {
    statusNote: /reconciliation|task reconciliation/i.test(currentStatus?.content ?? ""),
    taskCardsNote: /reconciliation|task reconciliation/i.test(taskCardsFile?.content ?? ""),
    hasReconMd: Boolean(reconFile),
    reconTitle: /NTTC Blueprint Task Reconciliation Report/i.test(reconFile?.content ?? ""),
    noDiskWrite:
  ...
  await new Promise((r) => setTimeout(r, 700));
  const dashRecon = await evaluate(`document.body.innerText`);
  report.dailyNextRecon = /Generate Task Reconciliation Report|Reconciliation Report/i.test(
    dashRecon,
  );
  ...
  if (!report.ui.reconciliationSection) fail("Reconciliation section missing");
  if (!report.ui.reconciliationAfterIntake) fail("Reconciliation not after intake");
  if (!report.noTaskCardsRecon.message) fail("No-task-cards reconciliation message missing");
  if (!report.contractFields.anyCardHasContracts) fail("Contract fields missing on task cards");
  if (!report.reconciliationGenerate.saved) fail("Reconciliation report not saved");
  if (!report.reconciliationGenerate.sectionsOk) fail("Reconciliation sections incomplete");
  if (!report.reconciliationGenerate.recommendationKnown) fail("Unknown recommendation label");
  if (!report.reconciliationCopy.copyLog) fail("Reconciliation copy log missing");
  if (!report.reconciliationClear.cleared) fail("Reconciliation clear failed");
  if (!report.reconciliationHistoryRestore.restored) fail("Reconciliation history restore failed");
  if (!report.reconciliationStale.stale) fail("Reconciliation not marked stale after regen");
  if (!report.planningPreview.noDiskWrite) fail("Planning preview wrote disk");
  if (!report.planningPreview.hasReconMd) fail("TASK_RECONCILIATION.md preview missing");
  if (!report.workflow.reconciliationStep) fail("Workflow reconciliation step missing");
