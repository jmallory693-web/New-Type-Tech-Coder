/**
 * Stage 87 launcher verification: bat starts packaged exe; task cards section spot-check.
 */
...
report.ui = {
  ...
  generateBtn: /Generate Phase Task Cards/i.test(body),
};
ws.close();
spawn("taskkill", ["/F", "/IM", "New Type Tech Coder.exe"], { stdio: "ignore" });
...
report.pass =
  ...
  !report.ui.usesVite &&
  report.ui.usesAsar &&
  report.ui.taskCardsSection &&
  report.ui.generateBtn;
