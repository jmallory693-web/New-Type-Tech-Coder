/**
 * Stage 59 packaged-app smoke harness (CDP).
 * Stage 58 Patch Draft Mode + Stage 56/54/52 regressions.
 */
import fs from "node:fs";
import path from "node:path";

const CDP = "http://127.0.0.1:9234";
const REPO =
...
const report = {
  ...
  stage58: {
    ...
    copyButton: null,
    checkboxes: null,
    noApplyPatchButton: null,
    confirmGateInBundle: null,
    packOnlyPrompt: null,
    noAiOnPackGenerate: null,
    optionalTogglesWork: null,
    patchDraftMapping: null,
    patchDraftModelSelect: null,
    roleHelpPatchDraft: null,
    previewAreaAfterIpc: null,
  },
  projectMemory: null,
...
function scanBundleStrings() {
  ...
  return {
    ...
    includeCodeAiCheckbox: haystack.includes(
      "Include latest Code AI response excerpt",
    ),
    includePlanDecisionCheckbox: haystack.includes(
      "Include Builder Plan / Decision Report excerpt",
    ),
    includeImplReviewCheckbox: haystack.includes(
      "Include Implementation Review excerpt",
    ),
    patchDraftNoApplyNote: haystack.includes(
      "NTTC will not edit files or apply patches",
    ),
    patchDraftConfirmTitle: haystack.includes(
      "Send this approved Code Context Pack to Local AI for a patch draft",
    ),
    confirmDraftOnly: haystack.includes("Output is a draft only"),
    confirmNoApply: haystack.includes("NTTC will not apply the patch"),
    confirmNoEditsPatch: haystack.includes("No source files will be edited"),
    noApplyPatchButton: !/\bApply Patch\b/i.test(haystack),
...
try {
  ...
  report.stage58.bundleStrings = {
    ...
    includeCodeAiCheckbox: bundle.includeCodeAiCheckbox,
    includePlanDecisionCheckbox: bundle.includePlanDecisionCheckbox,
    includeImplReviewCheckbox: bundle.includeImplReviewCheckbox,
    patchDraftNoApplyNote: bundle.patchDraftNoApplyNote,
    patchDraftConfirmTitle: bundle.patchDraftConfirmTitle,
    confirmDraftOnly: bundle.confirmDraftOnly,
    confirmNoApply: bundle.confirmNoApply,
    confirmNoEditsPatch: bundle.confirmNoEditsPatch,
    noApplyPatchButton: bundle.noApplyPatchButton,
    patchDraftMappingLabel: bundle.patchDraftMappingLabel,
  };
  for (const [key, ok] of Object.entries(report.stage58.bundleStrings)) {
    if (!ok) fail(`Stage 58 bundle string/check missing: ${key}`);
  }

  report.stage58.packOnlyPrompt = fs
    .readFileSync(
      path.join(
        REPO,
        "dist-electron",
        "main",
        "codeContext",
        "buildPatchDraftPrompt.js",
      ),
      "utf8",
    )
    .includes("NTTC Patch Draft") &&
    fs
      .readFileSync(
        path.join(
          REPO,
      ...
      .includes("NTTC did not edit source files");

  report.stage58.confirmGateInBundle =
    bundle.patchDraftConfirmTitle &&
    bundle.confirmDraftOnly &&
    bundle.confirmNoApply &&
    bundle.confirmNoEditsPatch;
  ...
  snap = await evaluate(
    `window.nttc.setCodeContextQuestion("Stage 59 smoke: draft patch safely.")`,
  );
  ...
  report.stage58.noAiOnPackGenerate =
    (snap.patchDraft?.saved ?? null) === null && !snap.patchDraft?.busy;
  ...
  report.stage58.generateButton = await evaluate(
    `[...document.querySelectorAll('button')].some((b) => /Generate Patch Draft with Local AI/i.test(b.textContent || ''))`,
  );
  report.stage58.copyButton = await evaluate(
    `[...document.querySelectorAll('button')].some((b) => /Copy Patch Draft/i.test(b.textContent || ''))`,
  );
  report.stage58.checkboxes = {
    codeAi: reportsText.includes("Include latest Code AI response excerpt"),
    planDecision: reportsText.includes(
      "Include Builder Plan / Decision Report excerpt",
    ),
    implReview: reportsText.includes("Include Implementation Review excerpt"),
  };
  ...
  snap = await evaluate(`window.nttc.setPatchDraftIncludeCodeAi(true)`);
  snap = await evaluate(
    `window.nttc.setPatchDraftIncludeBuilderPlanDecision(true)`,
  );
  snap = await evaluate(
    `window.nttc.setPatchDraftIncludeImplementationReview(true)`,
  );
  report.stage58.optionalTogglesWork =
    snap.patchDraft?.includeCodeAiResponseExcerpt === true &&
    snap.patchDraft?.includeBuilderPlanDecisionExcerpt === true &&
    snap.patchDraft?.includeImplementationReviewExcerpt === true;

  const patchDraftIpc = await evaluate(`window.nttc.generatePatchDraft()`);
  report.stage58.previewAreaAfterIpc = Boolean(
    patchDraftIpc.patchDraft?.saved?.previewExcerpt ||
      /provider is not ready|not found in the installed models|Generate a Code Context Pack/i.test(
        patchDraftIpc.patchDraft?.statusMessage || "",
      ),
  );
  if (patchDraftIpc.patchDraft?.saved) {
    report.stage58.savedMetadata = {
      modelName: patchDraftIpc.patchDraft.saved.modelName,
      roleMode: patchDraftIpc.patchDraft.saved.roleMode,
      generatedAt: patchDraftIpc.patchDraft.saved.generatedAt,
      contextPackGeneratedAt:
        patchDraftIpc.patchDraft.saved.contextPackGeneratedAt,
      mappingKey: patchDraftIpc.patchDraft.saved.mappingKey,
    };
  }
  ...
  report.stage58.patchDraftMapping = /Patch Draft/i.test(settingsText);
  report.stage58.patchDraftModelSelect = await evaluate(
    `[...document.querySelectorAll('select')].some((sel) => {
      const row = sel.closest('tr, .field-value, label, div');
      return row && /Patch Draft/i.test(row.textContent || '');
    })`,
  );

  const patchDraftHelp = await evaluate(`(async () => {
    const btn = [...document.querySelectorAll('.role-help-link')].find((el) =>
      (el.textContent || '').includes('Patch Draft'),
    );
    if (!btn) return { found: false, panel: false };
    btn.click();
    await new Promise((r) => setTimeout(r, 350));
  ...
  report.stage58.roleHelpPatchDraft = patchDraftHelp;
  ...
  if (!report.stage58.generateButton) fail("Generate Patch Draft button missing");
  if (!report.stage58.copyButton) fail("Copy Patch Draft button missing");
  for (const [key, ok] of Object.entries(report.stage58.checkboxes ?? {})) {
    if (!ok) fail(`Patch Draft checkbox missing: ${key}`);
  }
  if (!report.stage58.noApplyPatchButton) fail("Apply Patch button unexpectedly present");
  if (!report.stage58.confirmGateInBundle) {
    fail("Patch Draft confirmation gate strings missing from bundle");
  }
  if (!report.stage58.packOnlyPrompt) {
    fail("Patch Draft prompt builder missing draft-only guard");
  }
  if (!report.stage58.noAiOnPackGenerate) {
    fail("Pack preview generation should not create Patch Draft");
  }
  if (!report.stage58.optionalTogglesWork) fail("Patch Draft optional toggles failed");
  if (!report.stage58.previewAreaAfterIpc) {
    fail("Patch Draft IPC did not produce draft or expected blocked status");
  }
  if (!report.stage58.patchDraftMapping) fail("Patch Draft missing from Role Model Mapping");
  if (!report.stage58.patchDraftModelSelect) fail("Patch Draft model dropdown missing");
  if (!patchDraftHelp?.panel) fail("Patch Draft clickable help failed");
