/**
 * Stage 43 packaged-app smoke harness (CDP).
 * Confirms Stage 42 Implementation Review + Stage 40 comparison in packaged build.
...
const report = {
  ...
  generateImplementationReviewBtn: null,
  copyImplementationReviewBtn: null,
  reportsImplementationExcerpt: null,
  dailyNextImplementationKind: null,
...
try {
  ...
  report.generateImplementationReviewBtn = await evaluate(
    send,
    `!![...document.querySelectorAll('button')].find((b) => (b.textContent||'').includes('Generate Implementation Review'))`,
  );
  report.copyImplementationReviewBtn = await evaluate(
    send,
    `!![...document.querySelectorAll('button')].find((b) => (b.textContent||'').includes('Copy Implementation Review'))`,
  );
  ...
  const optionSet = new Set(Array.isArray(optionTexts) ? optionTexts : []);
  ...
  report.reportsImplementationExcerpt =
    /Latest Implementation Review/i.test(reportsText) ||
    /Implementation Review/i.test(reportsText);
  ...
  const snap = await evaluate(
    ...
    `typeof window.nttc === 'object' && window.nttc.getSnapshot
      ? window.nttc.getSnapshot().then((s) => ({
          ...
          hasGenerateImplementationReview:
            typeof window.nttc?.generateImplementationReview === 'function',
          liveQwen: s.qwen ?? null,
          pageUrl: location.href,
        }))
      : { bridgeMissing: true }`,
  );

  // Daily Next Action: verify generate-implementation-review kind exists in bundled code
  report.dailyNextImplementationKind = await evaluate(
    send,
    `fetch(document.querySelector('script[src]')?.src || '').then(r => r.text()).then(t => /generate-implementation-review|Generate Implementation Review/.test(t)).catch(() => false)`,
  );
  ...
  if (!snap?.hasComparison) fail("builderPlanComparison missing from snapshot");
  if (!snap?.hasImplementationReview) fail("implementationReview missing from snapshot");
  if (!snap?.hasGenerateImplementationReview) {
    fail("generateImplementationReview IPC missing from preload bridge");
  }
  if (snap?.liveQwen?.liveInspectEnabled) fail("Live Qwen should be disabled");
  ...
  if (!report.implementationReview) fail("Implementation Review section missing");
  if (!report.generateImplementationReviewBtn) {
    fail("Generate Implementation Review button missing");
  }
  if (!report.copyImplementationReviewBtn) {
    fail("Copy Implementation Review button missing");
  }
  if (!report.reportsImplementationExcerpt) {
    fail("Reports tab Implementation Review excerpt area missing");
  }
  if (!report.dailyNextImplementationKind) {
    fail("Daily Next Action Implementation Review wiring missing from bundle");
  }
