/**
 * Stage 109 packaged-app smoke — ReportsArchitectureSection extraction after Stage 108.
 * Launch: release/win-unpacked/New Type Tech Coder.exe --remote-debugging-port=9246
 */
import fs from "node:fs";
import path from "node:path";
import { spawn } from "node:child_process";

const CDP = "http://127.0.0.1:9246";
const REPO =
...
Stage 108 extraction smoke.

# Plan
Verify ReportsArchitectureSection wiring.

# Files changed
- src/renderer/components/ReportsArchitectureSection.tsx

# Implementation summary
Extracted architecture panels from App.tsx.
...
try {
  ...
  report.bundleIncludesSection =
    bundleText.includes("Architecture Health") &&
    bundleText.includes("Architecture Refactor Task Cards") &&
    bundleText.includes("Architecture Refactor Builder Handoff") &&
    bundleText.includes("Architecture Refactor Implementation Intake");
  if (!report.bundleIncludesSection) fail("Bundle missing architecture section strings");

  spawn("taskkill", ["/F", "/IM", "New Type Tech Coder.exe"], { stdio: "ignore" });
  await new Promise((r) => setTimeout(r, 1500));
  spawn(EXE, ["--remote-debugging-port=9246"], {
    cwd: REPO,
    detached: true,
    stdio: "ignore",
  });
  ...
  report.uiReports = {
    ...
    generateCards: /Generate Architecture Refactor Task Cards/i.test(reportsText),
    generateHandoff: /Generate Architecture Refactor Builder Handoff/i.test(reportsText),
  ...
  await new Promise((r) => setTimeout(r, 1500));
  s = await snap();
  report.stage108Regression.cardsGenerated = Boolean(
  ...
  s = await snap();
  report.stage108Regression.handoffGenerated = Boolean(
  ...
  await new Promise((r) => setTimeout(r, 800));
  s = await snap();
  const summary = s.architectureRefactorTaskImplementationIntake?.selectedReport?.summaryMarkdown ?? "";
  report.stage108Regression.intakeSaved = summary.includes(
  ...
  report.stage108Regression.projectMemoryMentionsRefactor =
  ...
  report.stage108Regression.planningPreviewHasImplMd = Boolean(implMd);
  ...
  if (!report.stage108Regression.healthGenerated) fail("Architecture Health generate failed");
  if (!report.stage108Regression.appTsxMonolithRisk) {
    report.warnings.push("Architecture Health may not flag App.tsx monolith risk clearly");
  }
  if (!report.stage108Regression.mainIndexMonolithRisk) {
    report.warnings.push("Architecture Health may not mention main/index.ts");
  }
  if (!report.stage108Regression.cardsGenerated) fail("Refactor task cards generate failed");
  if (!report.stage108Regression.handoffGenerated) fail("Builder handoff generate failed");
  if (!report.stage108Regression.intakeSaved) fail("Implementation intake save failed");
