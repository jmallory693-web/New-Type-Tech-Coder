/**
 * Stage 101 launcher verification — packaged app + Architecture Health accuracy spot-check.
 */
...
const s = await snap();
const md = s.architectureHealth?.saved?.markdown ?? "";
...
report.ui = {
  ...
  generateButton: /Generate Architecture Health Report/i.test(body),
  appInReport: /src\/renderer\/App\.tsx/.test(md) && /Critical monolith risk/i.test(md),
...
report.pass =
  ...
  report.ui.generateButton &&
  report.ui.appInReport &&
  report.ui.noDistElectron &&
