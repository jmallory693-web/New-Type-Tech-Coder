/**
 * Stage 132 launcher verification — packaged app after Stage 131 Local Planner Build Brief.
 */
...
const plannerBeforeSafety = await evaluate(`(() => {
  ...
  return plannerLabel >= 0 && safetyLabel > plannerLabel;
})()`);
...
report.ui = {
  ...
  plannerBeforeSafety: plannerBeforeSafety === true,
  generatePlannerBrief: buildButtons.some((b) =>
    /^Generate Planner Brief$/i.test(b),
  ),
  copyPlannerBrief: buildButtons.some((b) => /^Copy Planner Brief$/i.test(b)),
  clearPlannerBrief: buildButtons.some((b) => /^Clear Planner Brief$/i.test(b)),
  copyPasteNote:
    /copy\/paste into a local planner model/i.test(buildBody) &&
    /does not send it automatically/i.test(buildBody),
  noRunLocalPlanner: !buildButtons.some((b) => /^Run Local Planner$/i.test(b)),
...
report.pass =
  ...
  report.ui.plannerBeforeSafety &&
  report.ui.generatePlannerBrief &&
  report.ui.copyPlannerBrief &&
  report.ui.clearPlannerBrief &&
  report.ui.copyPasteNote &&
  report.ui.noRunLocalPlanner &&
  report.ui.noSendToAi &&
