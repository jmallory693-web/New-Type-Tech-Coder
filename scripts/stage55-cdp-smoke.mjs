/**
 * Stage 55 packaged-app smoke harness (CDP).
 * Stage 54 Code AI + Stage 52/50/48 regressions.
 */
...
function scanBundleStrings() {
  ...
  return {
    ...
    confirmSendPack: haystack.includes("Send this approved Code Context Pack to Local AI"),
    ...
    confirmNoHiddenAccess: haystack.includes("No hidden file access is granted"),
    packWarningCopy: haystack.includes("warnings or truncation"),
    previewOnlyNote: haystack.includes("does not send anything to AI"),
    packOnlySendNote: haystack.includes("cannot browse your project or edit files"),
...
try {
  report.stage54.bundleStrings = scanBundleStrings();
  for (const [key, ok] of Object.entries(report.stage54.bundleStrings)) {
    if (!ok) fail(`Stage 54/55 bundle string missing: ${key}`);
  }

  report.stage54.packOnlyPrompt = fs
    .readFileSync(
      path.join(REPO, "dist-electron", "main", "codeContext", "buildCodeContextAiPrompt.js"),
      "utf8",
    )
  ...
  const ollamaBefore = snap.provider?.lastTestAt ?? null;
  ...
  snap = await evaluate(
    `window.nttc.setCodeContextQuestion("Stage 55 smoke: explain selected code safely.")`,
  );
  ...
  report.stage54.ipcBlockedWithoutPack = Boolean(
    ...
    /provider is not ready|Generate a Code Context Pack/i.test(
  ...
  report.stage54.confirmStringsInBundle = report.stage54.bundleStrings.confirmSendPack;
  ...
  if (!report.stage54.askDisabledWithoutPack) {
    fail("Ask should be disabled or explain when no Code Context Pack exists");
  }
  if (!report.stage54.noAiOnPackGenerate) fail("Pack preview generation should not create Code AI response");
  if (!report.stage54.packOnlyPrompt) fail("Code AI prompt builder missing approved-pack-only guard");
