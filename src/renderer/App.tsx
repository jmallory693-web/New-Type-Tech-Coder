...
const emptySafeScaffoldFileContentPreview = {
  saved: null,
  busy: false,
  statusMessage:
...
const emptySafeScaffoldFinalConfirmation = {
  saved: null,
  busy: false,
  statusMessage:
      ...
      <main className="main-grid">
        ...
        {activeTab === "build" ? (
          <BuildModeTab
            ...
            onGenerateFileContentPreview={async () => {
              const next =
                await window.nttc.generateSafeScaffoldFileContentPreview();
              setSnapshot(next);
            }}
            onClearFileContentPreview={async () => {
              const next =
                await window.nttc.clearSafeScaffoldFileContentPreview();
              setSnapshot(next);
            }}
            onCopyFileContentPreview={async () => {
              const md = safeScaffoldFileContentPreview.saved?.markdown;
              if (!md) return;
              try {
                await navigator.clipboard.writeText(md);
                const next =
                  await window.nttc.recordCopySafeScaffoldFileContentPreview();
                setSnapshot(next);
              } catch {
                await window.nttc.logUiAction(
                  "warning",
            ...
            onRecordFinalConfirmation={async (acks) => {
              const next =
                await window.nttc.recordSafeScaffoldFinalConfirmation(acks);
              setSnapshot(next);
            }}
            onClearFinalConfirmation={async () => {
              const next =
                await window.nttc.clearSafeScaffoldFinalConfirmation();
              setSnapshot(next);
            }}
            onCopyFinalConfirmation={async () => {
              const md = safeScaffoldFinalConfirmation.saved?.markdown;
              if (!md) return;
              try {
                await navigator.clipboard.writeText(md);
                const next =
                  await window.nttc.recordCopySafeScaffoldFinalConfirmation();
                setSnapshot(next);
              } catch {
                await window.nttc.logUiAction(
                  "warning",
